...
def get_gmm_coeffs(gmm_params): 
    ...
    """
    gmm_mu = gmm_params[..., : ArgsConfig.HIDDENSIZE * ArgsConfig.NMIX] 
    gmm_mu.contiguous()
    gmm_pi_activ = gmm_params[..., ArgsConfig.HIDDENSIZE * ArgsConfig.NMIX :]
...
def mdn_loss(gmm_params, mu, stddev, batchsize):
    ...
    - the predicted distribution of the MDN (given by gmm_mu and gmm_pi) with - the target distribution created by the Encoder block (given by mu and
    stddev).
    """
    gmm_mu, gmm_pi = get_gmm_coeffs(gmm_params)
    eps = torch.randn(stddev.size()).normal_().cuda()
    z = torch.add(mu, torch.mul(eps, stddev))
    z_flat = z.repeat(1, ArgsConfig.NMIX)
    z_flat = z_flat.reshape(batchsize * ArgsConfig.NMIX, ArgsConfig.HIDDENSIZE)
    gmm_mu_flat = gmm_mu.reshape(batchsize * ArgsConfig.NMIX, ArgsConfig.HIDDENSIZE)
    dist_all = torch.sqrt(torch.sum(torch.add(z_flat, gmm_mu_flat.mul(-1)).pow(2).mul(50), 1))
    dist_all = dist_all.reshape(batchsize, ArgsConfig.NMIX)
